...
def get_gas_table(client_results, client, test_cases, gas, method, metadata):
    ...
    for test_case, _ in test_cases.items():
        ...
        if max_val == 0:
            gas_table[test_case][2] = f'N/A'
            gas_table[test_case][3] = f'N/A'
            gas_table[test_case][4] = f'N/A'
            gas_table[test_case][5] = f'N/A'
            gas_table[test_case][6] = f'N/A'
            gas_table[test_case][7] = f'N/A'
            gas_table[test_case][8] = f'N/A'
            gas_table[test_case][9] = f'N/A'
            gas_table[test_case][10] = f'N/A'
            continue
...
def get_gas_resume(client_results, client, test_cases, gas_set, method, metadata):
    ...
    for test_case, _ in test_cases.items():
        for gas in gas_set:
            if gas not in client_results[client][test_case]:
                continue
            results = [int(gas) / x * 1000 if x != 0 else 0 for x in client_results[client][test_case][gas][method]]
...
def process_results_3(client_results, clients, results_paths, test_cases, methods, gas_set, metadata):
    ...
    for client in clients:
        ...
        results_to_print += ('<thread>\n'
                             ...
                             f'<th onclick="sortTable(1, \'table_{client}\')" style="cursor: pointer;">Min (MGas/s) &uarr; &darr;</th>\n'
                             f'<th onclick="sortTable(2, \'table_{client}\')" style="cursor: pointer;">Max (MGas/s) &uarr; &darr;</th>\n'
                             '<th>p50 (MGas/s)</th>\n'
                             '<th>p95 (MGas/s)</th>\n'
                             '<th>p99 (MGas/s)</th>\n'
                             '<th>N</th>\n'
        ...
        results_to_print += ('</table>\n'
                             '</tbody>\n')
        # results_to_print += ('<h3>Worst Test Cases</h3>\n'
        #                      f'<table id="table_{client}_worst">'
        #                      '<thead>'
        #                      '<tr>'
        #                      '<th>Case</th>'
        #                      '<th>Title</th>'
        #                      f'<th onclick="sortTable(0, \'table_{client}_worst\')">Performance (MGas/s)</th>'
        #                      '</tr>'
        #                      '</thead>'
        #                      '<tbody>')
        ...
        #     gas_name = f'{gas}M'
        #     results_to_print += f'<tr>\n<td>{gas_name}</td>\n'
        #     results_to_print += f'<td>{resume[str(gas)][0]}</td>\n'
        #     results_to_print += f'<td>{resume[str(gas)][1]:.2f} MGas/s</td>\n</tr>\n'
